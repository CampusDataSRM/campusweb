...
const MAX_MASK_SIZE_LOW_END = 512;
const ADAPTIVE_SAMPLE_FRAMES = 60;
const ADAPTIVE_FPS_THRESHOLD = 34;
const QUALITY_TIERS = [
  { renderScale: 1, fpsCap: 0, glow: true },
  { renderScale: 0.8, fpsCap: 0, glow: true },
  { renderScale: 0.65, fpsCap: 30, glow: false }
] as const;
...
export default function ShapeWaves({
  ...
  useEffect(() => {
    ...
    const measureSurface = (): [number, number] => {
      dpr = Math.min(window.devicePixelRatio || 1, MAX_DPR) * QUALITY_TIERS[qualityTier].renderScale;
    ...
    void (async () => {
      try {
        ...
        const glowEnabled = () => settingsRef.current.glow > 0 && QUALITY_TIERS[qualityTier].glow;
        let lastIntro = false;
        ...
        const render = (now: number) => {
          ...
          const fpsCap = QUALITY_TIERS[qualityTier].fpsCap;
          if (animating && lastFrameTime && fpsCap > 0 && now - lastFrameTime < 1000 / fpsCap - 0.5) {
          ...
          if (animating && !introPlaying) {
            frameSamples.push(deltaSeconds);
            if (frameSamples.length >= ADAPTIVE_SAMPLE_FRAMES) {
              const average = frameSamples.reduce((total, sample) => total + sample, 0) / frameSamples.length;
              frameSamples = [];
              if (average > 1 / ADAPTIVE_FPS_THRESHOLD && qualityTier < QUALITY_TIERS.length - 1) {
                qualityTier += 1;
                frameId = requestAnimationFrame(render);
                resize();
                return;
        ...
        const drawMask = () => {
          ...
          const maskBudget = lowEnd ? MAX_MASK_SIZE_LOW_END : MAX_MASK_SIZE;
          const maskScale = hasText ? Math.min(1, maskBudget / Math.max(surfaceWidth, surfaceHeight)) : 0;
